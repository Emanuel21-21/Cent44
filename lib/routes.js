...
Router.route('/alumnos_notas/:_id', function(){
	let mesa = Mesas.findOne({_id: this.params._id});
	if (!mesa){
		Router.go('mesas_profesor');
	}
	else{
		this.render('alumnos_notas',{
			data: {
				mesa: mesa
			}
		})
...
/* Router.route('/alumnos_notas', {
	name: 'alumnos_notas'
})
 */
//-------------------------------SECCION MESAS ALUMNO------------------------
Router.route('/mesas_alumno',{
	name: 'mesas_alumno',
	data: {
		mesas(){
			return MesasIndex;
...
//-------------------------------SECCION MESAS PROFESOR------------------------
Router.route('/mesas_profesor',{
	name: 'mesas_profesor',
	data: {
		mesas(){
			return MesasIndex;
